...
- **Docs via Context7 MCP (recommended)** — when working on this repo, Claude should use the Context7 MCP server to fetch current docs for any library/SDK/CLI touched here (Cloudflare Workers, Drizzle, Vectorize, `@modelcontextprotocol/sdk`, `@cloudflare/workers-oauth-provider`, `@sentry/cloudflare`, Notion API, etc.) rather than relying on training data — even for well-known libs. Prefer it over web search for library docs. Skip for refactoring, business-logic debugging, or general programming concepts. Optional for forkers (no secrets, no auth required — just install the MCP server if you want it).
